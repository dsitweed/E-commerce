      ...
      <div className="singleProductRight">
        <div className="singleProductTags">
          {data.tags.map((tag, index) => {
            return <Link to={"/"} className="singleProductTag link" key={index}>{tag}</Link>;
          })}
        </div>
        ...
        <div className="SPAddContainer">
          <div className="SPAddAmount">
            <RemoveIcon className="SPAddIcon" onClick={() => {if (quantity > 1) setQuantity(quantity - 1)}}/>
            <input type="text" name="quantity" step="1" value={quantity} disabled/>
            <AddIcon className="SPAddIcon" onClick={() => setQuantity(quantity + 1)}/>
          </div>
          <div className="SPAddAction">
            <button className="SPAddBtn" style={{backgroundColor:"#ffb916"}}>Buy Now</button>
            <button className="SPAddBtn" style={{backgroundColor:"#f57224"}}>Add to Cart</button>
          </div>
        </div>
      </div> {/*end right single product*/}
    </div>
  );
};
